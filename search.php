...
$sql ="select question.*, users.name, users.surname from question inner join users on question.user = users.id and question.title like :ifade";
$stmt = $db->prepare($sql);
$stmt->execute(Array(":ifade"=>$search));
$pcs = $stmt->rowCount(); 
?>
<!DOCTYPE html>
<html lang="en">
...
<div class="baslik">
   ...
   <?php
     ...
     echo "</p>";
     if($pcs == 0){
         echo "<p style='color: brown;'> Aradıgınız ifadeye ait sonuç bulunamadı!</p>";
     } else {
         echo "<p> Toplam $pcs tane sonuç bulundu! </p>";
     }
   ?>
 </div>
